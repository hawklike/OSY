...
/*---------------------------------------------------------------------------*/
void   merge ( vector<int> & src, vector<int> & dst, int n, int thr )
{
  int block = n / thr;
  while ( block < n )
  {
    ...
    for ( int i = 0;  i < n; i += 2 * block )
      merge ( src . data () + i,  src . data () + min ( i + block, n ),
              src . data () + min ( i + block, n ), src . data () + min(i + 2*block, n ),
              dst . data () + i );
    swap ( src, dst );
    block *= 2;
  }
...
int main ( int argc, char * argv [] )
{
   ...
   */
  for ( int i = 0; i < thr; i ++ )
    threads . push_back ( thread ( thrFunc, src . data () + i * block, i == thr - 1 ? max - i * block : block ) );
